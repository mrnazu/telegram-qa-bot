...
async function postDailyQuestion() {
    try {
        getCurrentQuestionNumber(async (err, currentQuestionNumber) => {
            ...
            console.log(
                `Posting question ${currentQuestionNumber} at ${new Date().toISOString()}`,
            );

            const questionData = await fetchQuestion(currentQuestionNumber);

            if (questionData) {
                checkAndPostQuestion(
                    questionData,
                    currentQuestionNumber,
                    async (err) => {
                        if (err) {
                            console.error(
                                `Error posting question ${currentQuestionNumber}:`,
                                err,
                            );
                        } else {
                            const nextQuestionNumber =
                                (currentQuestionNumber % MAX_QUESTION_NUMBER) +
                                1;

                            await saveCurrentQuestionNumber(nextQuestionNumber);

                            console.log(
                                `Question number updated to ${nextQuestionNumber}`,
                            );
                        }
                    },
                );
                ...
                console.error(
                    `Failed to fetch question ${currentQuestionNumber}`,
                );
            }
        });
    } catch (error) {
...
connectToMongoDB()
    .then(() => {
        ...
        console.log("Cron job scheduled to run every night at 11 PM EAT.");
    })
    .catch((err) => {
